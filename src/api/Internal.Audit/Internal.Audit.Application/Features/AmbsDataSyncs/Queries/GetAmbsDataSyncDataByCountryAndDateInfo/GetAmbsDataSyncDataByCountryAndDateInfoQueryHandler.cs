...
public class GetAmbsDataSyncDataByCountryAndDateInfoQueryHandler : IRequestHandler<GetAmbsDataSyncDataByCountryAndDateInfoQuery, IEnumerable<GetAmbsDataSyncDataByCountryAndDateInfoDTO>>
{
    ...
    {
        var datas =await _dataSyncQueryRepository.GetDataSyncList( request.CountryId, request.startDate, request.endDate,request.typeId,request.conversionRate,request.pageNumber,request.pageSize);
