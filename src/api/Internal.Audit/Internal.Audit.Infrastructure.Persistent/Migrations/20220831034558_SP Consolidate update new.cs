...
{
    public partial class SPConsolidateupdatenew : Migration
    {
...
BEGIN
	...
	declare @totalCountBranch int
	SELECT  @totalCountBranch=count(*)
from 
security.Branch b
...
select *,@totalCountBranch as TC from(select l.*,f.AuditFrequencyType as AuditFrequency_Type from(select cast(@riskassesmentid as uniqueidentifier)RiskAssesmentId,*,([Loproductivity_Score]+[Fraud_Score]+[StaffTurnOver_Score]+[Overdue_Score]+[Collection_Score]+[Disbursement_Score])/6 [Avg_Score],
CASE 
...
FETCH NEXT  CASE when @pageSize=-1 then case when @totalCountBranch=0 then 1 else @totalCountBranch end else  @pageSize end ROWS ONLY;
drop table #t1
drop table #tr
END");
        }
